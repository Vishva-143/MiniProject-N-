...
print("Marks Migration: Fixed columns → Dynamic subjects/marks")
...
try:
    ...
    conn.commit()
    print(f"✅ Migration complete: {migrated} mark entries moved to dynamic table!")
    print("💡 Fixed columns preserved as backup. Drop manually if no longer needed.")
    
    # Verify
    cursor.execute("SELECT COUNT(*) as dynamic_marks FROM marks")
    dynamic_count = cursor.fetchone()[0]
    print(f"📊 New marks table: {dynamic_count} records")
